...
export class AppComponent implements OnInit, OnDestroy {
  ...
  replaceOne(): void {
    for (let i = 0; i < this.results.length; i++) {
      if (
        this.results[i].snippet.toLowerCase().includes(
          `<span class="searchmatch">${this.getValues.searchWord}</span>`
        )
      ) {
        this.results[i].snippet = this.results[i].snippet.toLowerCase().replace(
  ...
  replaceAll(): void {
    this.results = this.results.map((information) => {
      return {
        snippet: information.snippet.toLowerCase()
          .split(
            `<span class="searchmatch">${this.getValues.searchWord}</span>`.toLowerCase()
          )
          .join(
